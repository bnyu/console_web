<template>
    <el-table :data="userList" v-loading="loading" class="m-content">
        <el-table-column :label="$t('app.user.username')" prop="username" width="160">
            <template v-slot="s">
                <div class="username">
                    {{s.row.username}}
                </div>
        ...
        <el-table-column :label="$t('app.user.roleList')">
            <template v-slot="s">
                <span v-loading="s.row.loading">
                    <el-tag v-for="item in s.row.role_list" :key="item.role_id" class="role-tag"
                            disable-transitions>
                        {{item.role_name}}
                    </el-tag>
                </span>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            handleEdit(row) {
                this.$router.push({name: 'user-edit', params: {id: row.user_id}}).then()
            },

            handleDelete(row) {
                this.$confirm(this.$t('app.notice.actCanNotUndo'), this.$t('app.act.delete') + ' ' + row.username, {
