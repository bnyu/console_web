<template>
    <el-form label-position="left" label-width="120px" :model="form" ref="form" :rules="rules">
        <el-form-item prop="name" class="m-add-input">
            ...
            <el-input size="small" v-model="form.name" autocomplete="off" :placeholder="$t('app.notice.inputRoleName')"
        ...
        <el-form-item>
            <el-button size="small" type="primary" @click="handleAddNewRole" :disabled="searching" v-loading="loading">
                {{$t('app.act.confirm')}}
            </el-button>
            <el-button size="small" @click="handleBack" :disabled="searching||loading">{{$t('app.act.back')}}
            </el-button>
        </el-form-item>
    </el-form>
</template>
        ...
        data() {
            ...
            return {
                ...
                rules: {
                    ...
                    permits: [
                        {required: true, validator: validatePermitList}
                    ]
                }
            }
        },
        methods: {
            ...
            addNewRole(name, permits) {
                api.addRole(name).then(data => {
                    api.updateRolePermits(data.role_id, permits).then(() => {
                        this.loading = false
                        this.$message({
