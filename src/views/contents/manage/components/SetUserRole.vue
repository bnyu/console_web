<template>
    <el-form label-position="left" label-width="120px">
        ...
        <el-form-item>
            <el-button size="small" type="primary" @click="handleSave" :disabled="loading" v-loading="loading">
                {{$t('app.act.save')}}
            </el-button>
            <el-button size="small" @click="handleBack" :disabled="loading">{{$t('app.act.back')}}</el-button>
        </el-form-item>
    </el-form>
</template>
...
<script>
    ...
    export default {
        ...
        methods: {
            getUserInfo() {
                this.loading = true
                api.getUserInfos(this.id - 1, this.id).then(data => {
                    if (data.length === 1) {
            ...
            handleSave() {
                this.loading = true
                api.updateUserRoles(this.id, this.checkedList).then(() => {
                    this.loading = false
                    this.$message({
            ...
            getRoleList(offset, size) {
                this.loading = true
                api.getRoleList(offset, size).then(data => {
