    ...
    <el-table-column :label="$t('app.user.permitList')">
      <template v-slot="s">
        <span v-loading="s.row.loading">
          <el-tag
            v-for="item in s.row.permit_list"
            :key="item"
            class="permit-tag"
            disable-transitions
          >
...
export default {
  ...
  methods: {
    getRoleList() {
      this.loading = true;
      api
        .getRoleList((this.currentPage - 1) * this.pageSize, this.pageSize)
        .then((data) => {
    ...
    handleEdit(row) {
      this.$router
        .push({ name: "role-edit", params: { id: row.role_id } })
        .then();
    },
    ...
    handleDelete(row) {
      this.$confirm(
        ...
        this.$t("app.act.delete") + " " + row.role_name,
        {
